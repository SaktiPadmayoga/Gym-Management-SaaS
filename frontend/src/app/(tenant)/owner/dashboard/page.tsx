...
export default function DashboardOwner() {
  ...
  return (
    <div className="space-y-6 font-figtree pb-12">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-900 text-white p-6 rounded-2xl border border-zinc-800 shadow-xl relative overflow-hidden">
        ...
        <div className="space-y-1 relative z-10">
          ...
          <h1 className="text-3xl font-extrabold font-outfit tracking-tight">
            {getGreeting()}, Owner GymFit
          </h1>
          <p className="text-zinc-400 text-sm flex items-center gap-2">
            <Building2 size={16} className="text-zinc-500" />
            Memantau <span className="text-white font-bold">{summary.total_branches}</span> cabang aktif di sistem pusat.
          </p>
        </div>
        <div className="bg-zinc-800/80 backdrop-blur border border-zinc-700 px-4 py-3 rounded-xl flex items-center gap-3 relative z-10 self-start md:self-auto">
          <Calendar className="text-indigo-400" size={20} />
          <div className="text-left">
            <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Hari Ini</p>
            <p className="text-sm font-semibold">{dayjs().format("dddd, DD MMMM YYYY")}</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              ...
              <h3 className="text-2xl font-bold text-zinc-900 font-outfit tracking-tight">
                {formatRupiah(summary.revenue_this_month)}
              </h3>
            </div>
                  ...
                  return sortedBranches.map((bp) => {
                    ...
                    return (
                      <tr key={bp.id} className="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td className="py-4 px-4 text-right font-bold text-zinc-950">
                          {formatRupiah(bp.revenue_this_month)}
                        </td>
                      ...
                      <td className="py-3 px-2 text-center">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider ${
                            statusColor[tx.status] || "bg-zinc-100 text-zinc-700"
                          }`}
                        >
                          {tx.status}
                        </span>
                      </td>
                      <td className="py-3 px-2 text-sm font-bold text-zinc-950 text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {recent_check_ins.length > 0 ? (
            recent_check_ins.map((ci) => (
              <div 
                key={ci.id} 
                className="flex items-center gap-3 p-3 bg-zinc-50/50 hover:bg-zinc-50 rounded-xl border border-zinc-100 transition-colors"
              >
                {ci.member_avatar ? (
                  <img
                    src={ci.member_avatar}
                    alt={ci.member_name}
                    className="w-10 h-10 rounded-full object-cover ring-2 ring-zinc-100 shadow-sm"
                  />
                ) : (
                ...
                <div className="min-w-0 flex-1 space-y-0.5">
                  ...
                  <p className="text-[10px] text-zinc-400 font-medium">
                    {dayjs(ci.checked_in_at).format("DD MMM, HH:mm")} WIB
                  </p>
                </div>
              </div>
            ))
            ...
            <div className="col-span-full py-8 text-center text-sm text-zinc-400">
              Belum ada log scan masuk hari ini.
            </div>
          )}
        </div>
      </div>

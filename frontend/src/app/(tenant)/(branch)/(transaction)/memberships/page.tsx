    ...
    const columns: Column<MembershipDetail>[] = [
        {
            ...
            render: (item) => (
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-500 font-semibold text-sm shrink-0">{item.member?.name?.charAt(0).toUpperCase() ?? "?"}</div>
                    <div>
                        <button onClick={() => router.push(`/members/${item.member_id}`)} className="font-medium text-zinc-800 hover:underline text-left">
                            {item.member?.name ?? "—"}
                        </button>
        ...
        },
        {
            ...
            render: (item) => (
                <div>
                    ...
                    <p className="text-xs text-zinc-400">{item.unlimited_checkin ? "Unlimited" : `Sisa ${item.remaining_checkin_quota ?? "—"} visit`}</p>
                </div>
            ),
            width: "w-44",
        },
            ...
            render: (item) => (
                <div>
                    ...
                    {item.days_remaining !== null && item.days_remaining !== undefined && (
                        <p className={`text-xs font-semibold ${item.days_remaining <= 7 ? "text-red-500" : item.days_remaining <= 30 ? "text-amber-500" : "text-green-600"}`}>
                            {item.days_remaining > 0 ? `${item.days_remaining} hari lagi` : "Berakhir hari ini"}
                        </p>
                    )}
                </div>
            ),
        ...
        },
        {
            ...
            render: (item) => <span className="text-sm font-medium text-zinc-700">{item.total_checkins ?? 0}x</span>,
            width: "w-28",
        },
        {
            header: "Status",
            render: (item) => <span className={`rounded-lg px-2 py-1 text-xs font-medium capitalize ${statusColor[item.status] ?? "bg-zinc-100 text-zinc-500"}`}>{item.status_label ?? item.status}</span>,
            width: "w-24",
        },
    ];
            ...
            <div className="py-4">
                ...
                <div className="overflow-x-auto">
                    {isLoading ? (
                        ...
                        <CustomTable columns={columns} data={entries} onRowClick={(row) => router.push(`/members/${row.member_id}`)} />
                    )}
                </div>

                <div className="mt-4 text-sm text-zinc-500">
                    Showing {entries.length > 0 ? (page - 1) * perPage + 1 : 0} to {(page - 1) * perPage + entries.length} of {meta?.total ?? entries.length} data
                </div>

                <div className="mt-4">
                    <PaginationWithRows hasNextPage={!!meta?.next_page_url} hasPrevPage={!!meta?.prev_page_url} totalItems={meta?.total ?? entries.length} rowOptions={[15, 30, 50]} defaultRowsPerPage={perPage} />
                </div>
            </div>
        </FormProvider>
    );
    ...
    const columns: Column<MembershipDetail>[] = [
        ...
        },
        {
            ...
            render: (item) => (
                <div>
                    <p className="text-sm font-medium text-zinc-800">{item.plan?.name ?? "—"}</p>
                    <p className="text-xs text-zinc-400">{item.plan?.price ? `Rp ${Number(item.plan.price).toLocaleString("id-ID")}` : "—"}</p>
                </div>
            ),
            width: "w-44",
        },
        ...
        {
            header: "Cabang",
            render: (item) => <span className="text-sm text-zinc-600">{item.branch?.name ?? "—"}</span>,
            width: "w-36",
        },
        {
            header: "Status",
        ...
        {
            header: "Dibuat",
            render: (item) => <span className="text-xs text-zinc-400">{item.created_at ?? "—"}</span>,
            width: "w-32",
        },
    ];
...
export default function MembershipsPage() {
    const searchParams = useSearchParams();
    const [activeTab, setActiveTab] = useState<TabKey>(() => (searchParams.get("tab") as TabKey) || "active");
        ...
        <div className="font-figtree">
            ...
            <div className="rounded-xl bg-white border border-gray-500/20 px-6 py-4">
                ...
                <div className="breadcrumbs text-sm text-zinc-400 mb-4">
                    <ul>
                        <li>Transaksi</li>
                        <li className="text-aksen-secondary">Keanggotaan</li>
                    </ul>
                </div>
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-zinc-500">Kelola paket aktif dan riwayat keanggotaan member</p>
                    </div>
                </div>

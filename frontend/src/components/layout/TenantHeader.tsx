...
function BranchSwitcher({
    ...
    return (
        <div ref={ref} className="relative">
            <button
                onClick={() => setOpen((v) => !v)}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/20 bg-white/10 hover:bg-white/20 transition text-sm font-medium"
                    ...
                    {branches.map((branch) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5 shrink-0 ml-2">
                                {branch.is_main && (
                                    <span className="text-[10px] bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded">Main</span>
                                )}
                                {currentBranch?.id === branch.id && (
...
/* =========================
 * HELPER — apakah warna gelap atau terang
 * untuk menentukan warna teks otomatis
 * ========================= */
...
export default function TenantHeader() {
    ...
    // Ambil setting publik branch (primary_color, accent_color, logo_url)
    ...
    const textColor     = isDark ? "#ffffff" : "#18181b"; // white atau zinc-900
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3">
                {(logoUrl || publicSettings?.logo_url) ? (
                    <img
                        src={logoUrl ?? publicSettings?.logo_url}
                        alt={data.name}
                ...
                <div>
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold leading-tight" style={{ color: textColor }}>
                            {data.name}
                        </span>
                        <StatusBadge status={data.status} />
                    </div>
                    {data.subscription && (
                        <div className="text-xs leading-tight mt-0.5" style={{ color: subTextColor }}>

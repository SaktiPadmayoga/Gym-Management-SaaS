...
export default function TenantHeader() {
    ...
    const daysLeft = data.subscription_ends_at ? Math.ceil((new Date(data.subscription_ends_at).getTime() - Date.now()) / 86400000) : null;
    ...
    return (
        <header className="h-14 flex items-center justify-between px-6 font-figtree transition-colors duration-300" style={{ backgroundColor: primaryColor }}>
            ...
            <div className="flex items-center gap-3">
                ...
                {daysLeft !== null && daysLeft <= 0 && <div className="text-xs text-red-600 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg font-medium">Subscription expired</div>}
            </div>
        </header>
    );
}

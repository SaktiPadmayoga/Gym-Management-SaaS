...
import { ownerSidebarData, type OwnerSidebarItem } from "@/types/owner-sidebar-menu";
...
const OwnerSidebar: React.FC<OwnerSidebarProps> = ({ isOpen, pathname }) => {
  const [expandedItems, setExpandedItems] = useState<Record<string, boolean>>({});

  const findActiveItem = (items: OwnerSidebarItem[], currentPath: string): OwnerSidebarItem | null => {
  ...
  const getParentPaths = (items: OwnerSidebarItem[], targetId: string, currentPath: string[] = []): string[] => {
    for (const item of items) {
      if (item.isHeader) continue;
  ...
  useEffect(() => {
    const activeItem = findActiveItem(ownerSidebarData, pathname);
    if (activeItem) {
      const pathsToExpand = getParentPaths(ownerSidebarData, activeItem.id);
  ...
  const isActive = (item: OwnerSidebarItem): boolean => {
    if (!item.path) return false;
    return pathname === item.path || pathname.startsWith(item.path + '/');
  };

  const isParentActive = (item: OwnerSidebarItem): boolean => {
  ...
  const renderChildren = (children: OwnerSidebarItem[]) => {
  ...
  const renderMenuItem = (item: OwnerSidebarItem) => {
    // Render header
    if (item.isHeader) {
  ...
  return (
    ...
    >
      ...
      <div className="pb-4 px-4 font-figtree">
        {ownerSidebarData.map(renderMenuItem)}
      </div>
    </aside>
  );
};

...
import { DollarSign, CreditCard, Activity, TrendingUp, Star } from "lucide-react";
import {
...
export default function FinanceTab({ data, startDate, endDate, onFilterChange }: FinanceTabProps) {
  ...
  useEffect(() => {
    ...
    else if (diffDays === 6 && end.isSame(now, "day")) detectedPeriod = "7d";
    else if (diffDays === 29 && end.isSame(now, "day")) detectedPeriod = "30d";
  ...
  const bestDay = revenueTrend.length > 0
    ? revenueTrend.reduce((best, cur) => toNumber(cur.revenue) > toNumber(best.revenue) ? cur : best)
    : null;
  ...
  return (
    <div className="space-y-6 mt-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
        ...
        {period === "custom" && (
          <div className="flex items-center gap-2">
            <input type="date" value={localCustomRange.start}
            ...
            <input type="date" value={localCustomRange.end}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-500/20">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-blue-50 text-blue-600 rounded-lg"><CreditCard size={20} /></div>
          </div>
        </div>

        {/* AOV — Revenue ÷ Transaksi, nol query baru */}
        <div className="bg-white p-5 rounded-xl border border-gray-500/20">
          <div className="flex justify-between items-start">
            <div>
            ...
            <div className="p-3 bg-violet-50 text-violet-600 rounded-lg"><TrendingUp size={20} /></div>
          </div>
        </div>

        {/* Hari Terbaik — max dari revenue_trend, nol query baru */}
        <div className="bg-white p-5 rounded-xl border border-gray-500/20">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-zinc-500 font-atkin tracking-tighter">Transaksi Terbanyak</p>
              <h3 className="text-2xl font-bold text-zinc-900 mt-1 font-outfit">{bestDay?.date ?? "-"}</h3>
              <p className="text-xs text-zinc-400 mt-1 font-atkin tracking-tighter">
                {bestDay ? formatRupiah(toNumber(bestDay.revenue)) : "Belum ada data"}
              </p>
            </div>
            <div className="p-3 bg-amber-50 text-amber-600 rounded-lg"><Star size={20} /></div>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {(tables?.recent_transactions || []).length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-6 text-zinc-500">Tidak ada transaksi di periode ini.</td>
                  </tr>
                ) : (

...
export default function DetailPtSession() {
    ...
    const handleMarkComplete = () => {
        markCompleteMutation.mutate({ id, notes: completionNotes || undefined }, {
            onSuccess: () => {
                ...
                <div className="font-figtree rounded-xl bg-white border px-6 py-4 border-zinc-200 shadow-sm min-h-[500px]">
                    ...
                    {['scheduled', 'ongoing'].includes(sessionData?.status ?? '') && (
                        <div className="mb-8 p-5 bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-200 rounded-2xl">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="ml-auto px-2 py-0.5 bg-teal-100 text-teal-700 text-xs font-bold rounded-full uppercase">
                                    {sessionData?.status}
                                </span>
                            </div>
                            <div className="mb-4">
                    ...
                    {['scheduled', 'ongoing'].includes(sessionData?.status ?? '') && (
                        isTrainer ? (
                            ...
                            <div className="flex flex-col gap-6 max-w-4xl mt-6 p-5 border border-zinc-100 bg-zinc-50/50 rounded-2xl">
                                <div>
                                ...
                                <div className="grid grid-cols-1">
                                    <TextInput 
                                        name="notes" 
                                        label="Alasan Reschedule / Catatan *" 
                                        placeholder="Tulis alasan rescheduling atau fokus latihan sesi ini..." 
                                        rules={{ required: "Catatan reschedule / alasan wajib diisi" }}
                                    />
                                </div>
                            </div>
                        ) : (

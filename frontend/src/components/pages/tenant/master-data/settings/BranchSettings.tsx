...
function AppearanceForm({ defaults, onSave, onReset, isSaving, isResetting }: { defaults: Record<string, any>; onSave: (data: any) => void; onReset: () => void; isSaving: boolean; isResetting: boolean }) {
        ...
        <FormProvider {...form}>
            <form onSubmit={form.handleSubmit(onSave)} className="space-y-6">
                ...
                <div className="flex gap-4 items-center">
                    <div className="w-12 h-12 rounded-xl border border-zinc-200 shadow-sm" style={{ backgroundColor: primaryColor ?? "#4F46E5" }} title="Primary color preview" />
                    <div className="w-12 h-12 rounded-xl border border-zinc-200 shadow-sm" style={{ backgroundColor: accentColor ?? "#7C3AED" }} title="Accent color preview" />
                ...
                <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-4">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="color" {...form.register("primary_color")} className="w-10 h-10 rounded cursor-pointer border border-zinc-200" />
                    ...
                    <div className="col-span-4">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="color" {...form.register("accent_color")} className="w-10 h-10 rounded cursor-pointer border border-zinc-200" />
                        ...
                        {days.map(({ key, label }) => (
                            <div key={key} className="grid grid-cols-12 items-center gap-3">
                                <div className="col-span-2 flex items-center gap-2">
                                    <input type="checkbox" {...form.register(`operating_hours.${key}.is_open`)} className="rounded" />
                                    <span className="text-sm text-zinc-700 w-20">{label}</span>
                                </div>
                                <div className="col-span-2">
                                    <input type="time" {...form.register(`operating_hours.${key}.open`)} className="w-full border border-zinc-200 rounded-lg px-3 py-1.5 text-sm text-zinc-700" />
                                </div>
                                <span className="text-zinc-400 text-sm">to</span>
                                <div className="col-span-2">
                                    <input type="time" {...form.register(`operating_hours.${key}.close`)} className="w-full border border-zinc-200 rounded-lg px-3 py-1.5 text-sm text-zinc-700" />
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <form onSubmit={form.handleSubmit(onSave)} className="space-y-6">
                ...
                <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-12">
                        ...
                        <input type="text" {...form.register("expiry_reminder_days")} className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm text-zinc-700" placeholder="3,7,14" />
                    </div>
                    <div className="col-span-12">
                        <label className="block text-sm font-medium text-zinc-700 mb-1">
                        ...
                        <textarea {...form.register("expiry_message_template")} rows={3} className="w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm text-zinc-700 resize-none" />
                    </div>
                </div>
...
function FormActions({ isSaving, isResetting, onReset }: { isSaving: boolean; isResetting: boolean; onReset: () => void }) {
    return (
        <div className="flex items-center justify-between pt-2">
            <button type="button" onClick={onReset} disabled={isResetting} className="text-sm text-zinc-400 hover:text-red-500 transition disabled:opacity-50">
                {isResetting ? "Resetting..." : "Reset to default"}
            </button>
            <CustomButton type="submit" disabled={isSaving} className="bg-aksen-secondary text-white px-4 py-2 disabled:opacity-50">
                    ...
                    {TABS.map((tab) => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition border-b-2 -mb-px ${
                                activeTab === tab.key ? "border-aksen-secondary text-aksen-secondary" : "border-transparent text-zinc-500 hover:text-zinc-700"
                            }`}
                        >
                            {tab.label}
                        </button>

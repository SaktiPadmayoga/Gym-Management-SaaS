...
export default function OwnerMemberTab({ data, isFiltered = false }: { data: any; isFiltered?: boolean }) {
    if (!data || !data.summary) return <div className="p-8 text-center text-zinc-500">Tidak ada data tersedia</div>;
    ...
    const branchDistribution = isFiltered
        ? []
        : (charts?.branch_distribution || []).map((item: any) => ({
              ...item,
              value: toNumber(item.value),
          }));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-500/20 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h3 className={`text-2xl font-bold ${netGrowth >= 0 ? "text-green-600" : "text-red-600"}`}>{netGrowth > 0 ? `+${netGrowth}` : netGrowth}</h3>
                    </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-500/20 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h3 className={`text-2xl font-bold ${parseFloat(churnRate) > 10 ? "text-red-600" : "text-zinc-900"}`}>{churnRate}%</h3>
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-500/20">
                    ...
                    <div className="h-64">
                        {registrationTrend.length === 0 ? (
                            <div className="flex items-center justify-center h-full text-zinc-400 text-sm">Tidak ada pendaftaran di periode ini.</div>
                        ) : (
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-500/20">
                    ...
                    <div className="h-64">
                        {planDistribution.length === 0 ? (
                            <div className="flex items-center justify-center h-full text-zinc-400 text-sm">Tidak ada data distribusi plan.</div>
                        ) : (
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={planDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" nameKey="name">
                ...
                {branchDistribution.length > 0 && (
                    <div className="bg-white p-6 rounded-xl border border-gray-500/20">
                        ...
                        <div className="h-64">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={branchDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" nameKey="name">
                                        ...
                                        <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50/50 text-sm">
                                            ...
                                            {!isFiltered && (
                                                <td className="py-3 px-2">
                                                    <span className="text-xs bg-zinc-100 text-zinc-600 px-2 py-1 rounded-md font-medium">{m.branch_name}</span>
                                                </td>
                                            )}
                                            <td className="py-3 px-2">
                                                <span
                                                    className={`text-xs font-semibold px-2 py-1 rounded ${
                                                        m.status === "active" ? "bg-green-100 text-green-700" : m.status === "inactive" ? "bg-gray-100 text-gray-600" : "bg-orange-100 text-orange-700"
                                                    }`}
                                                >
                                                    {m.status ?? "-"}
                                                </span>

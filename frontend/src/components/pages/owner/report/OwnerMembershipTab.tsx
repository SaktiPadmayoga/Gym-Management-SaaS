...
import { CheckCircle, AlertCircle, AlertTriangle, Snowflake } from "lucide-react";
...
export default function OwnerMembershipTab({ data, isFiltered = false }: { data: any; isFiltered?: boolean }) {
    if (!data || !data.summary) {
        return <div className="p-8 text-center text-zinc-500">Tidak ada data tersedia</div>;
    }
    ...
    return (
        <div className="space-y-6 mt-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-9 gap-4">
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4 col-span-6">
                    <div className="bg-white p-5 rounded-xl border border-gray-500/20 col-span-2">
                ...
                {planDistribution.length > 0 && (
                    <div className="bg-white p-6 rounded-xl border border-gray-500/20 col-span-3 font-figtree">
                        ...
                        <div className="h-40">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={planDistribution} cx="50%" cy="50%" innerRadius={45} outerRadius={65} paddingAngle={5} dataKey="value" nameKey="name">
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-500/20 font-figtree">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-zinc-900">Membership Akan Expired (7 Hari)</h2>
                                ...
                                expiringSoon.map((sub: any, idx: number) => (
                                    <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50/50 text-sm">
                                        ...
                                        <td className="py-3 px-2 text-right">
                                            <span className={`font-bold ${sub.days_left <= 3 ? "text-red-600" : "text-orange-500"}`}>{sub.days_left === 0 ? "Hari Ini" : `${sub.days_left} Hari Lagi`}</span>
                                        </td>
                                    </tr>
                                ))
                            )}
            ...
            {frozenList.length >= 0 && (
                <div className="bg-white p-6 rounded-xl border border-gray-500/20">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <h2 className="text-lg font-semibold text-zinc-900">Membership Frozen (Cuti)</h2>
                        <span className="ml-auto text-xs text-zinc-400">{frozenList.length} membership</span>
                    </div>
                    <p className="text-xs text-zinc-400 mb-4 ml-7">Member yang sedang cuti — follow up untuk reaktivasi.</p>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-100 text-xs text-zinc-500 uppercase tracking-wider">

...
import { CheckCircle2, Dumbbell, CreditCard, Lock, Sparkles, ArrowRight, ArrowLeft, Loader2, AlertCircle } from "lucide-react";
...
function RegisterTenantContent() {
    ...
    const onSubmit = async (data: RegisterTenantFormValues) => {
        try {
            ...
            const msg = err?.response?.data?.message || "Gagal memproses pendaftaran. Periksa kembali data Anda.";
            toast.error(msg);
        }
    };
    ...
    return (
        <div className="h-screen bg-slate-50 flex font-sans selection:bg-teal-200 overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        <h1 className="text-5xl font-black uppercase tracking-tighter leading-[0.9] mb-6">
                            Upgrade Bisnis <br />
                            <span className="text-teal-400">Fitness Anda.</span>
                        </h1>
                        <p className="text-slate-400 font-medium max-w-md leading-relaxed mb-10">Langkah terakhir untuk mengotomatisasi manajemen member, pembayaran, dan laporan operasional harian gym Anda.</p>

                        <ul className="space-y-5">
                            {["Setup sistem instan setelah pembayaran", "Input data awal bisa dibantu tim support", "Akses penuh ke modul manajemen", "Dukungan CS 24/7"].map((item, idx) => (
            ...
            <div className="flex-1 flex items-center justify-center p-3 sm:p-2 lg:p-0 min-h-0 h-screen overflow-hidden">
                <div className="w-full max-w-2xl">
                    <div className="p-2 sm:p-6 lg:p-5 min-h-0 flex flex-col">
                        {/* HEADER & PROGRESS TEXT */}
                        <div className="mb-4 flex justify-between items-end">
                            <div>
                                <h2 className="text-2xl font-black tracking-tighter uppercase text-slate-900 mb-1">{step === 1 ? "Pilih Paket" : "Detail Bisnis"}</h2>
                                <p className="text-sm font-bold text-slate-500 uppercase tracking-tight">{step === 1 ? "Langkah 1 dari 2: Profil & Layanan" : "Langkah 2 dari 2: Konfigurasi Sistem"}</p>
                            </div>
                        </div>
                        ...
                        <FormProvider {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col">
                                <AnimatePresence mode="wait">
                                    ...
                                    {step === 1 && (
                                        <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="flex-1 space-y-4">
                                                ...
                                                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                                                    ...
                                                    <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-lg">
                                                        <button
                                                            ...
                                                            className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-md transition-all ${
                                                                billingCycle === "monthly" ? "bg-white shadow-sm text-slate-900" : "text-slate-400"
                                                            }`}
                                                        >
                                                            Bln
                                                        </button>
                                                        ...
                                                        <button
                                                            ...
                                                            className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-md transition-all ${
                                                                billingCycle === "yearly" ? "bg-white shadow-sm text-slate-900" : "text-slate-400"
                                                            }`}
                                                        >
                                                            Thn
                                                        </button>
                                            ...
                                            <div className="pt-2 mt-auto">
                                                <CustomButton
                                                    ...
                                                    className="w-full h-12 bg-slate-900 hover:bg-slate-800 text-white font-black uppercase tracking-widest text-sm rounded-2xl transition-all flex items-center justify-center gap-2"
                                                >
                                    ...
                                    {/* ================= STEP 2: GYM DETAILS ================= */}
                                    {step === 2 && (
                                        <motion.div key="step2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="flex-1 flex flex-col space-y-4">
                                            <div className="space-y-3">
                                            ...
                                            <div className="pt-2 mt-auto flex gap-3">
                                                <CustomButton type="button" onClick={() => setStep(1)} className="w-12 h-12 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl flex items-center justify-center shrink-0">
                                                    <ArrowLeft className="w-5 h-5" />
                                                </CustomButton>

                                                <CustomButton
                                                    type="submit"
                                                    className="flex-1 h-12 bg-slate-950 hover:bg-slate-800 text-white font-black uppercase tracking-widest text-sm rounded-2xl shadow-xl shadow-slate-900/20 transition-all flex items-center justify-center gap-2"
                                                    disabled={registerPaidMutation.isPending}
                                                >
                                                    {registerPaidMutation.isPending ? (
                                                        "Memproses..."
                                                    ) : (
                                                        <>

...
import { LayoutDashboard, FileText, GitBranch, Globe, CreditCard, Users, User, Activity, Search, MoreVertical, ArrowUpRight, ChevronDown, Lock } from "lucide-react";
...
const DashboardPreview = () => {
    ...
    return (
        <section id="overview" className="relative py-24 md:py-24 px-6 bg-slate-50 font-sans overflow-hidden">
            {/* Background Pattern */}
            ...
            <div className="relative z-10 max-w-6xl mx-auto flex flex-col items-center">
                ...
                <div className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-6 shadow-sm text-slate-600">
                        <span className="w-2 h-2 rounded-full  bg-teal-500 animate-pulse" />
                        Live Interface
                    </div>
                    <h2 className="text-4xl md:text-4xl font-black tracking-tighter uppercase text-slate-900 mb-8">
                    ...
                    <div className="inline-flex bg-white border border-slate-200 rounded-xl p-1.5 shadow-sm overflow-x-auto max-w-full">
                        {previewTabs.map((tab) => {
                            ...
                            return (
                                ...
                                >
                                    {isActive && <motion.div layoutId="activeTabBg" className="absolute inset-0 bg-teal-500 rounded-lg" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />}
                ...
                <motion.div
                    ...
                    className="w-full rounded-[1.5rem] md:rounded-[1.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-slate-200 bg-white flex flex-col h-[650px]"
                >
                    {/* Browser / App Header */}
...
const SidebarItem = ({ icon: Icon, label, isActive = false }: SidebarItemProps) => (
    <div className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${isActive ? "bg-teal-400 text-white shadow-sm shadow-teal-500/20" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>
...
const MembersPage = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="p-6 md:p-6">
        {/* Breadcrumb */}
        ...
        {/* Page Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 md:gap-3">
            <div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-white">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button className="w-7 h-7 flex items-center justify-center rounded bg-teal-600 text-white text-xs font-bold shadow-sm">1</button>
                </div>
            </div>
        </div>
    </motion.div>
...
const DashboardPage = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="p-6 md:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-3">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                ...
                <p className="text-[10px] font-bold text-emerald-600 flex items-center gap-1">
                    <ArrowUpRight size={12} /> +14.5% vs last month
                </p>
            </div>
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
...
const SubscriptionPage = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="p-6 md:p-6">
        ...
        <div className="bg-white border border-red-200 rounded-xl p-6 shadow-sm mb-6 relative overflow-hidden">
            ...
            <button className="bg-red-50 text-red-600 border border-red-200 font-semibold px-4 py-2 rounded-lg text-sm hover:bg-red-100 transition-colors">View Pricing Plans</button>
        </div>

        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <h3 className="text-base font-bold text-slate-900 mb-4">Billing History</h3>
            <div className="text-center py-12 text-slate-400 text-sm">No payment history available.</div>
        </div>
    </motion.div>
);

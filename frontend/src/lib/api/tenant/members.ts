...
import { AssignMemberBranchRequest, MemberCreateRequest, MemberData, MemberUpdateRequest, UpdateMembershipRequest } from "@/types/tenant/members";

export const membersAPI = {
    getAll: async (params?: { page?: number; per_page?: number; search?: string; status?: string; gender?: string; is_active?: boolean; expiring_in_days?: number }): Promise<MemberData[]> => {
    ...
    // Branch membership
    getBranches: async (id: string): Promise<MemberData[]> => {
        const response = await tenantApiClient.get(`/members/${id}/branches`);
        return response?.data.data ?? [];
    },

    assignBranch: async (id: string, payload: AssignMemberBranchRequest): Promise<MemberData> => {
        const response = await tenantApiClient.post(`/members/${id}/branches`, payload);
        return response?.data.data;
    },

    updateMembership: async (memberId: string, branchId: string, payload: UpdateMembershipRequest): Promise<MemberData> => {
        const response = await tenantApiClient.patch(`/members/${memberId}/branches/${branchId}/membership`, payload);
        return response?.data.data;
    },
